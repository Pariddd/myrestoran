...
class MenuController extends Controller
{
    ...
    {
        Session::forget('cart');
        return redirect()->route('cart')->with('success', 'Keranjang berhasil dihapus.');
    }
    ...
    public function storeOrder(Request $request)
    {
        ...
        foreach ($cart as $itemId => $item) {
            OrderItem::create([
                ...
                'total_price' => ($item['price'] * $item['qty']) + (0.1 * $item['qty'] * $item['price']),
            ]);
        }

        Session::forget('cart');

        return redirect()->route('menu')->with('success', 'Pesanan berhasil diproses. Terima kasih!');
    }
}
